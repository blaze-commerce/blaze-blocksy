  ...
  $(document).ready(function () {
	...
	function bindHover() {
		if ($(window).width() >= 768) {
		  ...
		  $('.blz-img-group').hover(
			function() {
			  $(this).find('.blz-hovered-img').hide();
			  $(this).find('.blz-hover-img').show();
			},
			function() {
			  $(this).find('.blz-hovered-img').show();
			  $(this).find('.blz-hover-img').hide();
			}
		  );
	...
	// Re-check when resizing (optional)
	$(window).on('resize', function() {
		bindHover();
	});
	
	function appendRewardsCard() {
        // Only append if not already there
        if ($('.blaze-register-checkout-info').length === 0) {
            var rewardsHtml = `
                <div class="blaze-register-checkout-info">
                    <div class="blz-register-KN-rewards-card">
                        <div class="blz-KN-reward-card">
                            <h6>Join the KN Point & Rewards Program</h6>
                            <p class="sub-heading">PLACE AN ORDER AND EARN POINTS THAT YOU CAN SPEND ON FUTURE ORDERS.</p>
                            <p>Check the create an account checkbox and earn 400 points. If you are an existing customer, please login before picking a new order.</p>
                        </div>
                    </div>
                </div>
            `;
            $('.create-account').append(rewardsHtml);
        }
    }
    ...
    // Re-append after checkout updates
    $(document.body).on('updated_checkout', appendRewardsCard);
