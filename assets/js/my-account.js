...
jQuery(document).ready(function ($) {
   ...
   * Uses CSS transitions instead of jQuery fadeIn/fadeOut for better performance
   */
  function initTemplate2FormSwitching() {
    var $loginContainer = $(".blaze-login-register.template2 .login-container");
    var $registerContainer = $(
      ".blaze-login-register.template2 .register-container"
    );

    // Initially hide the register form for template2
    $registerContainer.addClass("is-hidden");

    // Show register form
    $(".show-register-form").on("click", function (e) {
      e.preventDefault();
      $loginContainer.addClass("is-hidden");
      // Wait for fade out transition to complete
      setTimeout(function () {
        $registerContainer.removeClass("is-hidden");
      }, 300);
    });
  ...
  function initFormValidation() {
    ...
    $(".blaze-login-register form").on("submit", function (e) {
      ...
      if (!isValid) {
        e.preventDefault();
        // Scroll to first error using native smooth scroll (more performant)
        var firstError = $form.find(".error").first();
        if (firstError.length) {
          var firstErrorEl = firstError[0];
  ...
  console.log(
    "✅ Blocksy Child My Account functionality initialized successfully"
  );
});
