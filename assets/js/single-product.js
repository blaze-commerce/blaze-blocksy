...
(function ($) {
  ...
  function initReviewFormValidation() {
    ...
    submitBtn.addEventListener("click", function (e) {
      ...
      form.querySelectorAll(".blaze-field-invalid").forEach(function (el) {
      ...
      if (author && author.hasAttribute("required") && !author.value.trim()) {
        errors.push({ field: author, message: "Please enter your name." });
      }

      // Email
      var email = form.querySelector('input[name="email"]');
      if (email && email.hasAttribute("required") && !email.value.trim()) {
        errors.push({
          field: email,
  ...
  function handleReviewCommentHash() {
    ...
    var reviewsTab = document.querySelector('[data-target="#tab-reviews"]');
    if (reviewsTab) {
      reviewsTab.click();
    }
    ...
    $(".wcsatt-option-card .subscription-price").each(function () {
      ...
      Array.prototype.slice.call(el.childNodes).forEach(function (node) {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          node.textContent = node.textContent.replace(/\s*—\s*save\s*/g, "");
        }
      });
