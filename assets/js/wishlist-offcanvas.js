...
(function ($) {
  ...
  function openWishlistOffCanvas() {
    ...
    // CRITICAL: Remove inert attribute to enable interactions
  ...
  function handleOffCanvasWishlistActions() {
    // Handle sign up button click - using capture phase to intercept before Blocksy
    document.addEventListener(
      "click",
      function (e) {
        const button = e.target.closest(
          ".wishlist-guest-notice .notice-signup",
        );
        if (button) {
          e.preventDefault();
          e.stopPropagation();
  ...
  $(document).on("click", function (e) {
    if ($("body").hasClass("wishlist-offcanvas-open")) {
      ...
      if (!$panel.is(e.target) && $panel.has(e.target).length === 0) {
        ...
        // Re-add inert attribute when closing
        $panel.attr("inert", "");
      }
    }
  });
  ...
  /**
   * Listen for Blocksy panel close events to re-add inert
   */
  if (typeof ctEvents !== "undefined") {
    ctEvents.on("ct:overlay:handle-close", function (data) {
      const panel = document.querySelector(SELECTORS.PANEL);
      if (panel && data && data.container === panel) {
