...
(function ($) {
	...
	function getColorValue(colorObj) {
		...
		if (colorObj.default && colorObj.default.color) return colorObj.default.color;
	...
	wp.customize('productStockFont', function (value) {
		value.bind(function (newValue) {
			...
			if (newValue.size) {
				const size = typeof newValue.size === 'object' ? newValue.size.desktop : newValue.size;
			...
			if (newValue.variation) {
				...
				updateCSSVariable(el, '--theme-font-weight', weight === '00' ? '400' : weight);
			...
			if (newValue['letter-spacing']) {
				updateCSSVariable(el, '--theme-letter-spacing', newValue['letter-spacing']);
			}

			if (newValue['text-transform']) {
				updateCSSVariable(el, '--theme-text-transform', newValue['text-transform']);
			}

			if (newValue['text-decoration']) {
				updateCSSVariable(el, '--theme-text-decoration', newValue['text-decoration']);
			}
		});
	});
