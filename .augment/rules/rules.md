...
- **Guides:** `docs/guides`
- **Configuration:** `docs/guides/config`
...
- **Command:** `augment validate --rules validate-documentation --changed`
...
- **Type:** `docs_location`
- **Root:** `docs`
- **Allowed Subdirectories:** api, components, modules, guides, config, migrations
- **Severity:** Error
...
When `require-docs-for-code-changes` rule is violated:

1. **Scaffold Document**
   - **API Changes:** Use `api_endpoint` template
   - **Function/Method Changes:** Use `function_method` template
   - **Configuration Changes:** Use `configuration_change` template
   - **New File/Module:** Use `new_file_or_module` template
   - **Default:** Use `generic_code_change` template
...
*This configuration ensures comprehensive documentation coverage for all code changes while providing automated scaffolding and validation to maintain quality standards.*
