...
<div class="wcsatt-options-wrapper <?php echo esc_attr( implode( ' ', $wrapper_classes ) ); ?>"
	...
	<div class="wcsatt-options-product-wrapper" <?php echo in_array( 'closed', $wrapper_classes ) ? 'style="display:none;"' : ''; ?>>
		...
		if ( $display_dropdown ) {
			...
			if ( $dropdown_label ) {
				?>
				<label class="wcsatt-options-product-dropdown-label" for="<?php echo esc_attr( $select_id ); ?>">
				...
				<label class="wcsatt-options-product-dropdown-label screen-reader-text"
					...
					<?php esc_html_e( 'Select subscription option', 'woocommerce-all-products-for-subscriptions' ); ?>
				</label>
				<?php
			}
			...
			<select class="wcsatt-options-product-dropdown" id="<?php echo esc_attr( $select_id ); ?>"
				...
				foreach ( $options as $option ) {
					...
					?>
					<option <?php echo $option['selected'] ? 'selected="true"' : ''; ?>value="
				<?php echo esc_attr( $option['value'] ); ?>">
						<?php echo esc_html( $option['dropdown'] ); ?>
					</option>
					<?php
				}
				?>
					...
					<label>
						...
						<div class="wcsatt-option-card-content">
							<div class="wcsatt-option-card-header">
								<span class="wcsatt-option-card-label">
									<?php echo esc_html( $card_label ); ?>
								</span>
								<?php if ( $card_badge ) : ?>
									<span class="wcsatt-option-card-badge">
										<?php echo esc_html( $card_badge ); ?>
									</span>
								<?php endif; ?>
							</div>
							<span
								class="wcsatt-option-card-price <?php echo esc_attr( $option['class'] ); ?>-price <?php echo esc_attr( $option['class'] ); ?>-details">
								<?php
								// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								echo $card_price;
								?>
						...
						<?php if ( $card_terms ) : ?>
							<p class="wcsatt-option-card-terms">
								<?php echo esc_html( $card_terms ); ?>
							</p>
						<?php endif; ?>
					</label>
				</li>
