...
function blocksy_child_get_pickup_locations( $order ) {
	...
	if ( empty( $pickup_locations ) ) {
		...
		// Build address parts
		if ( $address_1 )
			$store_address_parts[] = $address_1;
		if ( $address_2 )
			$store_address_parts[] = $address_2;
		if ( $city )
			$store_address_parts[] = $city;
		if ( $state_name )
			$store_address_parts[] = $state_name;
		if ( $postcode )
			$store_address_parts[] = $postcode;
		if ( $country_name && $country_name !== $state_name )
			$store_address_parts[] = $country_name;
				...
				<div class="blaze-commerce-order-details-container">
					...
					<div class="blaze-commerce-order-confirmation-container">
						...
						<p class="blaze-commerce-order-number">
							You will receive your confirmation email to
						...
						<div class="resend-email-container">
							...
							<button type="button" class="button wp-element-button" id="resend-email-btn"
...
function blocksy_child_blaze_commerce_addresses_section( $order ) {
	...
	<div class="blaze-commerce-addresses-section">
		...
		<div
			class="blaze-commerce-addresses-grid blaze-commerce-addresses-flex <?php echo $is_pickup ? 'pickup-billing' : 'shipping-billing'; ?>">
...
function blocksy_child_blaze_commerce_account_creation( $order ) {
	...
	<div class="blaze-commerce-account-creation">
		...
		<form class="blaze-commerce-account-form" method="post"
			action="<?php echo esc_url( wc_get_endpoint_url( 'order-received', $order->get_id(), wc_get_checkout_url() ) ); ?>">
			<div class="blaze-commerce-form-field">
				...
				<input type="text" id="account_first_name" name="account_first_name"
					value="<?php echo esc_attr( $order->get_billing_first_name() ); ?>" required>
			</div>

			<div class="blaze-commerce-form-field">
				<label for="account_last_name">Last name *</label>
				<input type="text" id="account_last_name" name="account_last_name"
					value="<?php echo esc_attr( $order->get_billing_last_name() ); ?>" required>
			</div>
	...
	<div class="blaze-commerce-summary-content">
		...
		foreach ( $items as $item_id => $item ) {
			...
			<div class="blaze-commerce-product-item">
				<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $product->get_name() ); ?>"
					class="blaze-commerce-product-image">
		...
		<div class="blaze-commerce-summary-breakdown">
			...
			<div class="blaze-commerce-summary-row">
				...
				<span
					class="blaze-commerce-summary-value"><?php echo blocksy_child_get_shipping_display( $order ); ?></span>
			</div>
			...
			<?php if ( $order->get_total_tax() > 0 ) : ?>
				<div class="blaze-commerce-summary-row">
					...
					<span
						class="blaze-commerce-summary-value">$<?php echo number_format( $order->get_total_tax(), 2 ); ?></span>
				</div>
			<?php endif; ?>
		</div>
