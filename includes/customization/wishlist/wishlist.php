...
/**
 * Blaze Product Image - Wishlist AJAX Handlers
 *
 * @since 1.0.0
 */
class Blaze_Product_Image_Wishlist_Ajax {

	/**
	 * Constructor
	 */
	public function __construct() {
		add_action( 'wp_ajax_blaze_add_to_wishlist', array( $this, 'add_to_wishlist' ) );
		add_action( 'wp_ajax_nopriv_blaze_add_to_wishlist', array( $this, 'add_to_wishlist' ) );
		add_action( 'wp_ajax_blaze_remove_from_wishlist', array( $this, 'remove_from_wishlist' ) );
		add_action( 'wp_ajax_nopriv_blaze_remove_from_wishlist', array( $this, 'remove_from_wishlist' ) );
	}
	...
	public function add_to_wishlist() {
		check_ajax_referer( 'blaze_wishlist_nonce', 'nonce' );
		...
		// Check if Blocksy wishlist extension exists
		if ( class_exists( 'BlocksyChildWishlistHelper' ) ) {
			$wishlist_ext = BlocksyChildWishlistHelper::get_wishlist_extension();

			if ( $wishlist_ext ) {
				// Use Blocksy's wishlist system
				$wishlist_ext->add_product_to_wishlist( $product_id );
				wp_send_json_success( array( 'message' => __( 'Product added to wishlist.', 'blocksy-child' ) ) );
			}
		}

		// Fallback: use custom wishlist storage
		...
		wp_send_json_error( array( 'message' => __( 'Product already in wishlist.', 'blocksy-child' ) ) );
	}
	...
	public function remove_from_wishlist() {
		...
		if ( class_exists( 'BlocksyChildWishlistHelper' ) ) {
			...
			if ( $wishlist_ext ) {
				// Use Blocksy's wishlist system
				$wishlist_ext->remove_product_from_wishlist( $product_id );
				wp_send_json_success( array( 'message' => __( 'Product removed from wishlist.', 'blocksy-child' ) ) );
			}
		}
		...
		wp_send_json_error( array( 'message' => __( 'Product not in wishlist.', 'blocksy-child' ) ) );
	}

	/**
	 * Get wishlist from session/cookie
	 *
	 * @return array Wishlist product IDs.
	 */
	private function get_wishlist() {
		if ( isset( $_COOKIE['blaze_wishlist'] ) ) {
			$wishlist = json_decode( stripslashes( $_COOKIE['blaze_wishlist'] ), true );
...
new Blaze_Product_Image_Wishlist_Ajax();
